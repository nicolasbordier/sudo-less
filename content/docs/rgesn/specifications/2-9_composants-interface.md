...
[Alt Impact](https://altimpact.fr/ecoconception-du-site/) (site porté par l'ADEME, audit Conserto) illustre un troisième cas de figure, à mi-chemin entre les deux précédents : un thème développé spécifiquement pour l'occasion, avec des composants « open source et bien optimisés », mais une auto-critique honnête sur un composant tiers embarqué — le calculateur d'empreinte carbone [impactco2.fr](https://impactco2.fr/) intégré à une page du site « n'est pas optimisé (66 requêtes au chargement + 2 Mo de données) et alourdit considérablement l'impact environnemental de la page », l'audit concluant explicitement que « les modalités d'intégration de ce service sont à revoir ». Avoir bien conçu son propre design system n'empêche donc pas un widget tiers mal intégré de rester un point faible identifié mais non résolu.

L'épisode 12 du talk-show [Instant Coding](https://www.youtube.com/watch?v=uEp7hPIRBC0), partage un cas très concret de composant remis en question plutôt qu'intégré tel quel : le métier demandait d'afficher sur la page d'accueil d'un site public une carte de France interactive prête à l'emploi, pesant à elle seule **3 Mo** — chargée par défaut par l'ensemble des visiteurs, alors que la page d'accueil est le point d'entrée le plus fréquenté du site. L'équipe a challengé le besoin réel plutôt que d'accepter le composant tel quel : la carte a été conservée (le besoin métier étant réel), mais déplacée derrière une action explicite de l'utilisateur plutôt que chargée automatiquement à l'arrivée sur la page — ne consommant les 3 Mo que pour les visiteurs qui en ont effectivement besoin. Le même retour d'expérience cite un deuxième réflexe plus général, au sujet des cartes interactives type Google Maps utilisées uniquement pour afficher une adresse : une simple image statique (voire le texte de l'adresse seul) suffit dans ce cas précis, sans justifier le poids d'un composant cartographique interactif complet.
