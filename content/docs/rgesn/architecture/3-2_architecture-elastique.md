...
Les mécanismes ci-dessus ajustent les ressources en temps réel selon la charge. Une approche complémentaire, plus ponctuelle, consiste à **redimensionner périodiquement une VM à taille fixe** au vu de son usage réel constaté — ce qui répond directement à la question du GR491 ci-contre, « Le nombre de cœurs attribués est-il fonction de l'usage réel ? ». La plupart des plateformes proposent un outil de recommandation dédié plutôt que de laisser cette analyse à la charge de l'équipe :

- **VMware Aria Operations** (vROPS, déjà cité au [critère 3.7](../3-7_environnements-developpement-test/) pour la détection des VM inactives) dispose aussi d'une fonction de [détection des VM surdimensionnées](https://docs.vmware.com/en/vRealize-Operations/8.10/com.vmware.vcom.user.doc/GUID-62358711-BEA7-4C26-8BB2-8247DDEE03E2.html) : par défaut, une VM est signalée comme surdimensionnée lorsque sa consommation de CPU, mémoire ou disque reste sous 50 % de sa capacité allouée sur la période analysée — pertinent pour un parc de VM on-premises à taille fixe, à l'inverse de l'auto-scaling dynamique décrit ci-dessus.
- [**AWS Compute Optimizer**](https://aws.amazon.com/compute-optimizer/) analyse 14 jours de métriques CloudWatch (CPU, mémoire, réseau, disque, EBS) pour classer chaque instance EC2 en sous-dimensionnée, surdimensionnée, optimale ou inactive, et recommander un type d'instance plus adapté.
- [**Azure Advisor**](https://learn.microsoft.com/en-us/azure/advisor/advisor-cost-recommendations) applique un seuil documenté — utilisation CPU ≤ 5 % et trafic réseau ≤ 7 Mo pendant 4 jours ou plus — pour identifier les VM sous-utilisées et recommander un redimensionnement ou une extinction.
- [**GCP Recommender**](https://docs.cloud.google.com/compute/docs/instances/apply-machine-type-recommendations-for-instances) génère quotidiennement des recommandations de type de machine à partir de 8 jours de métriques Cloud Monitoring, avec des gains annoncés de l'ordre de **20 à 40 %** sur le coût de calcul à performance équivalente.

Les deux approches ci-dessus — auto-scaling temps réel et rightsizing périodique — sont présentées séparément ici car ce sont deux mécanismes distincts, mais certaines plateformes commerciales les unifient en un seul moteur continu plutôt que de traiter chaque plateforme (VM, conteneurs, cloud, on-premises) séparément : [**IBM Turbonomic**](https://www.ibm.com/products/turbonomic) (ex-Turbonomic, racheté par IBM en 2021) analyse en continu la demande réelle des applications pour à la fois redimensionner les VM et conteneurs (rightsizing), replacer les charges de travail (placement) et ajuster les répliques Kubernetes, sur un périmètre hybride couvrant VMware, AWS, Azure, GCP et OpenShift/Kubernetes. Chaque action proposée est simulée et validée au regard des contraintes de performance avant exécution — automatique ou soumise à validation selon la politique définie. IBM revendique une réduction moyenne de **33 %** du gaspillage cloud et infrastructure chez ses clients, un chiffre à prendre comme une donnée commerciale plutôt qu'une mesure indépendante.
...
L'épisode 12 du talk-show [Instant Coding](https://www.youtube.com/watch?v=uEp7hPIRBC0), partage un retour d'expérience sur un site public à destination de professionnels (consultation de données, sans transaction), hébergé sur [Scalingo](https://scalingo.com/) : le conteneur de **production** lui-même est **scalé à zéro entre 22h et 7h** via l'API de la plateforme, l'usage du service étant limité aux horaires de bureau de ses utilisateurs professionnels. Point notable : le client a accepté l'extinction nocturne (7 h sur 24, soit environ 30 % du temps) mais a refusé l'extinction complète du week-end par précaution, certains utilisateurs pouvant s'y connecter en dehors des horaires normaux — un compromis pragmatique plutôt qu'une élasticité totale. L'argument de réduction de coûts (facturation à la minute chez Scalingo) a d'ailleurs pesé autant que l'argument environnemental pour emporter l'adhésion du client. Le même retour d'expérience illustre aussi le rightsizing du côté conception : le conteneur a été dimensionné sur le plus petit gabarit disponible (taille « S » chez Scalingo) et validé par un test de charge artisanal (1000 requêtes envoyées d'affilée), sans qu'un gabarit supérieur ait jamais été nécessaire pour environ 80 utilisateurs.
